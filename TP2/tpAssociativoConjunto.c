...
//Perguntar ao prof se e necessario atribuir valores
typedef struct
{
    int *palavras;
    int endBloco;
...
int tamanhoPrograma = 10002;//qde de instrucoes
...
void guardarCaches(int posicaoCache, int tamanhoCache, int conjuntoCache1, int conjuntoCache2, int conjuntoCache3,  Endereco *endAux);
...
BlocoMemoria* buscarNasMemorias(Endereco* end){
    ...
    for(int i = conjuntoCache3 * 4; i < conjuntoCache3 * 4 + 4; i++){
        if(!isAtualizadoCache(i, cache3)){
            ...
            guardarCaches(posicaoCache2, tamanhoCache2, conjuntoCache1, conjuntoCache2, conjuntoCache3, end);
    ...
    for(int i = conjuntoCache2 * 4; i < conjuntoCache2 * 4 + 4; i++){
        if(!isAtualizadoCache(i, cache2)){
            ...
            guardarCaches(posicaoCache1, tamanhoCache1, conjuntoCache1, conjuntoCache2, conjuntoCache3, end);
    ...
    guardarCaches(posicaoCache3, tamanhoCache3, conjuntoCache1, conjuntoCache2, conjuntoCache3, end);
